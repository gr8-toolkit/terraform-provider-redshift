...
- `temporary_credentials` (Block List, Max: 1) Configuration for obtaining temporary credentials. Use `cluster_identifier` for provisioned clusters or `workgroup_name` for Redshift Serverless. (see [below for nested schema](#nestedblock--temporary_credentials))
