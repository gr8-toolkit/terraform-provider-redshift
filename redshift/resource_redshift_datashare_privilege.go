...
func redshiftDatasharePrivilege() *schema.Resource {
	return &schema.Resource{
		...
		CreateContext: RedshiftResourceFuncContext(resourceRedshiftDatasharePrivilegeCreate),
		ReadContext:   RedshiftResourceFuncContext(resourceRedshiftDatasharePrivilegeRead),
		DeleteContext: RedshiftResourceFuncContext(resourceRedshiftDatasharePrivilegeDelete),
...
func resourceRedshiftDatasharePrivilegeCreate(db *DBConnection, d *schema.ResourceData) error {
	...
	if consumerNamespaceSet {
		...
		return fmt.Errorf("exactly one of %s or %s is required", datasharePrivilegeNamespaceAttr, datasharePrivilegeAccountAttr)
	}
	log.Printf("[DEBUG] %s\n", query)
...
AND
  consumer_namespace = $2`
	...
	if err == sql.ErrNoRows {
		log.Printf("[WARN] Redshift Datashare privilege for namespace (%s) not found", consumerNamespace)
		d.SetId("")
		return nil
	}
	if err != nil {
	...
	return d.Set(datasharePrivilegeShareDateAttr, shareDate)
}
...
AND
  consumer_account = $2`
	...
	if err == sql.ErrNoRows {
		log.Printf("[WARN] Redshift Datashare privilege for account (%s) not found", consumerAccount)
		d.SetId("")
		return nil
	}
	if err != nil {
